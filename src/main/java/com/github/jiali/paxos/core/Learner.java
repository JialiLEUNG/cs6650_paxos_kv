...
import com.github.jiali.paxos.packet.LearnRequest;
import com.github.jiali.paxos.packet.LearnResponse;
...
public class Learner {
	...
	public Learner(int id, List<NodeInfo> learners, NodeInfo my, NodeConfiguration nodeConfiguration, Accepter accepter,
	...
	private void sendRequest(int id, int instance) {
		...
		byte[] data;
		try {
			data = this.objectSerialize.objectToObjectArray(new Packet(packetBean, this.groupId, NodeType.LEARNER));
	...
	private void sendResponse(int peerId, int instance, Value value) {
		...
		try {
			...
			this.client.sendTo(peer.getHost(), peer.getPort(),
					this.objectSerialize.objectToObjectArray(new Packet(packetBean, this.groupId, NodeType.LEARNER)));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
