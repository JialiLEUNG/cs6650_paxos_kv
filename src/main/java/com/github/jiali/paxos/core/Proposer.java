...
import com.github.jiali.paxos.packet.PrepareRequest;
...
public class Proposer {
	...
	public Proposer(int id, List<NodeInfo> acceptors, NodeInfo my, int timeout, Accepter accepter, int groupId,
					PaxosClient client) {
		this.id = id;
	...
	private void prepare(int id, int instance, int ballot) {
		...
		try {
			PacketBean bean = new PacketBean("PrepareRequest", new PrepareRequest(id, instance, ballot));
			byte[] msg = this.objectSerialize.objectToObjectArray(new Packet(bean, groupId, NodeType.ACCEPTER));
	...
	private void accept(int id, int instance, int ballot, Value value) {
		...
		try {
			PacketBean bean = new PacketBean("AcceptRequest", new AcceptRequest(id, instance, ballot, value));
			byte[] msg = this.objectSerialize.objectToObjectArray(new Packet(bean, groupId, NodeType.ACCEPTER));
			this.acceptors.forEach((info) -> {
				try {
					this.client.sendTo(info.getHost(), info.getPort(), msg);
